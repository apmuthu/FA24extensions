...
function employees_table() {
	...
	if(db_has_employee()) {
		...
		$sql = get_employees_sql(check_value('show_inactive'), get_post('DeptId'));
...
function employee_settings($cur_id) {
	...
	if($cur_id) {
		if($employee['emp_hiredate'] != '0000-00-00')
			department_list_row(_('Department').':', 'DepartmentId', null, _('Not selected'));
		else {
	...
	salaryscale_list_row(_('Salary').':', 'EmpSalary', null, _('Not selected'));
	if($cur_id) {
