	...
	foreach ($locationsList as $location) {
		if (empty($date))
			...
			try {
				...
				$fromdate = gmdate("Y-m-d\TH:i:s\Z", strtotime($date));
				$todate = gmdate("Y-m-d\TH:i:s\Z");
				$cursor = null;
				$allPayments = array();
				do {
					$apiResponse = $paymentsApi->listPayments($fromdate, $todate, null, $cursor, $location->id, null);
					if ($apiResponse->isSuccess()) {
						$listPaymentsResponse = $apiResponse->getResult();
						$payments = $listPaymentsResponse->getPayments();
						if (!is_null($payments))
							$allPayments = array_merge($payments, $allPayments);
					} else {
			...
			$loc_orders[$location->id] = array($location->name, count($allPayments));
		}
	}
		...
		if ($action == 'o_import') { // Import Order specified by oID
			...
			if ($error == 0) {
                ...
                try {
                    ...
                    display_notification("Error while calling listLocations: " . $e->getMessage());
                    die();
                }
                ...
                foreach ($locationsList as $location) {
                    ...
                    try {
                        $paymentsApi = $client->getPaymentsApi();
                        $fromdate = gmdate("Y-m-d\TH:i:s\Z", strtotime($_POST['from_date']));
                        $todate = gmdate("Y-m-d\TH:i:s\Z", strtotime("+1 day", strtotime($_POST['to_date'])));
                        $cursor = null;
                        do {
                            $apiResponse = $paymentsApi->listPayments($fromdate, $todate, null, $cursor, $location->id, null);
                            if ($apiResponse->isSuccess()) {
...
// display_notification(print_r($t, true));
                            ...
                            if ($payment_method == "CARD")
                                $cc_info = $t->getCardDetails()->getCard()->getCardBrand()  . " " . $t->getCardDetails()->getCard()->getLast4();
                            ...
                            $refundedMoney = $t->getRefundedMoney();
							if (!is_null($refundedMoney) && $refundedMoney->getAmount() != 0) {
							...
							$dt = DateTime::createFromFormat(SQUARE_RFC3339, $t->getCreatedAt(), new DateTimeZone('UTC'));
                            ...
                            // TBD: where to find out if tax was included?
...
*/
                                try {
                                    $item3 = $cat_api->retrieveCatalogObject($item->getCatalogObjectId(), false);
                                } catch (Exception $e) {
                                    display_notification('Exception when calling CustomersApi->retrieveCustomerCatalogObject on '. $item->getName() . ":" . $e->getMessage());
...
*/
                            ...
                            $tipMoney = $t->getTipMoney();
							if (!is_null($tipMoney) && $tipMoney->getAmount() != 0)
