...
// date_:	2019-03-14
// Title:	Enhanced Aged Customer Balances
// Based On rep102.php - this module will replace it
// ----------------------------------------------------------------
$path_to_root="..";
...
function getTransactions($debtorno, $todate, $show_also_allocated)
{
    $sql = "SELECT trans.*, memo_,
    ...
    $value = "IFNULL($sign*(trans.ov_amount + trans.ov_gst + trans.ov_freight + trans.ov_freight_tax + trans.ov_discount - trans.alloc),0)";
    $due = "IF (trans.type=".ST_SALESINVOICE.", trans.due_date, trans.tran_date)";
    $sql .= "IF ((TO_DAYS('$todate') - TO_DAYS($due)) > 0,$value,0) AS Due,
              IF ((TO_DAYS('$todate') - TO_DAYS($due)) > $past1,$value,0) AS Overdue1,
                IF ((TO_DAYS('$todate') - TO_DAYS($due)) > $past2,$value,0) AS Overdue2";

    $sql .= " FROM ".TB_PREF."debtor_trans trans
            LEFT JOIN ".TB_PREF."comments c ON c.type = trans.type AND c.id = trans.trans_no
			WHERE tran_date <= '$todate' AND debtor_no = ".db_escape($debtorno)."
...
function print_aged_customer_analysis()
{
	...
	while ($myrow=db_fetch($result))
	{
		...
		$custrec = get_customer_details($myrow['debtor_no'], $to, false);
		if (!$custrec)
			continue;
		...
		if (!$summaryOnly)
		{
            ...
            if ($trans['type'] == ST_SALESINVOICE || $trans['type'] == ST_BANKPAYMENT ||
                ($trans['type'] == ST_JOURNAL && $trans["TotalAmount"] > 0.0))
                $balance += $DisplayTotal;
            else
                $balance -= $DisplayTotal;

            if ($trans['tran_date'] < date2sql($startdate))
                continue;
            ...
            if ($first) {
                ...
                $rep->TextCol(7, 8, number_format2($prev_balance, $dec), -2);
            }
				$rep->NewLine(1, 2);
