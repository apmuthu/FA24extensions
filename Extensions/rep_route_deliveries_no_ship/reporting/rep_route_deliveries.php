...
function get_delivery_date_range($from, $to, $route)
{
	...
	$sql = "SELECT trans.debtor_no, trans.branch_code, trans.trans_no, trans.reference";
	if ($route == 1)
	...
	if ($route == 1)
		$sql .= " LEFT JOIN ".TB_PREF."addfields_cust addfields ON trans.branch_code = addfields.cust_branch_no" ;
...
function print_deliveries()
{
	...
	// Shipper ie. Driver requires modified function in core.
	...
	if ($route == 1)
	{
		...
		if (file_exists($path_to_root ."/modules/additional_fields/includes/addfields_db.inc"))
		{
		...
		$route_config = include_once($path_to_root ."/modules/rep_route_deliveries/route_config.php");
