...
function print_stock_check()
{
	...
	if ($check)
	{
		$cols = array(0, 75, 225, 250, 295, 345, 390, 435, 485, 545);
		$headers = array(_('Stock ID'), _('Description'), _('UOM'), _('Quantity'), _('Check'), _('Demand'), $available, _('On Order'), _('Bin Location'));
		$aligns = array('left',	'left',	'left', 'right', 'right', 'right', 'right', 'right', 'right');
	}
	else
	{
		$cols = array(0, 75, 225, 250, 315, 380, 435, 485, 545);
		$headers = array(_('Stock ID'), _('Description'), _('UOM'), _('Quantity'), _('Demand'), $available, _('On Order'), _('Bin Location'));
	...
	while ($trans=db_fetch($res))
	{
		...
		$the_bin_no_is = get_item_bin_no($trans['stock_id'], $loc_code);
